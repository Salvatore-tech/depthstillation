...
from dataloader import caddy_loader

OUTPUT_DIR = "output"

"""Create output directories to store artefacts"""


def create_output_directories():
    if not os.path.exists(os.path.join(OUTPUT_DIR, "im0")):
        os.makedirs(os.path.join(OUTPUT_DIR, "im0"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "im1_raw")):
        os.makedirs(os.path.join(OUTPUT_DIR, "im1_raw"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "im1")):
        os.makedirs(os.path.join(OUTPUT_DIR, "im1"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "flow")):
        os.makedirs(os.path.join(OUTPUT_DIR, "flow"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "flow_color")):
        os.makedirs(os.path.join(OUTPUT_DIR, "flow_color"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "depth_color")):
        os.makedirs(os.path.join(OUTPUT_DIR, "depth_color"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "instances_color")):
        os.makedirs(os.path.join(OUTPUT_DIR, "instances_color"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "H")):
        os.makedirs(os.path.join(OUTPUT_DIR, "H"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "M")):
        os.makedirs(os.path.join(OUTPUT_DIR, "M"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "M'")):
        os.makedirs(os.path.join(OUTPUT_DIR, "M'"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "P")):
        os.makedirs(os.path.join(OUTPUT_DIR, "P"))
    if not os.path.exists(os.path.join(OUTPUT_DIR, "H'")):
        os.makedirs(os.path.join(OUTPUT_DIR, "H'"))
...
parser.add_argument("--data_path", type=str, help="Dataset path", default='./samples/')
...
parser.add_argument("--change_motion", dest="change_motion", action="store_true",
...
raw_images, depths = caddy_loader.caddy_loader(args.data_path)
...
for i in range(0, len(raw_images)):
    ...
    depth = cv2.imread(depths[i], -1) / (2 ** 16 - 1)
    ...
    for idm in range(args.num_motions):
        ...
        flow_16bit = cv2.cvtColor(
            np.concatenate((flow_01 * 64. + (2 ** 15), np.ones_like(flow_01)[:, :, :, 0:1]), -1)[0], cv2.COLOR_BGR2RGB)
        ...
        # Get the current image name (without the extension)
        current_image_name = os.path.basename(raw_images[i]).split('.')[0]

        # Save images
        cv2.imwrite(os.path.join(OUTPUT_DIR, "im0", current_image_name + ".jpg"), rgb[0].numpy())
        cv2.imwrite(os.path.join(OUTPUT_DIR, "im1_raw", current_image_name + "_%02d.jpg" % (idm)), im1_raw)
        cv2.imwrite(os.path.join(OUTPUT_DIR, "im1", current_image_name + "_%02d.jpg" % (idm)), im1)
        cv2.imwrite(os.path.join(OUTPUT_DIR, "flow", current_image_name + "_%02d.png" % (idm)),
                    flow_16bit.astype(np.uint16))
        cv2.imwrite(os.path.join(OUTPUT_DIR, "H", current_image_name + "_%02d.png" % (idm)), masks["H"] * 255)
        cv2.imwrite(os.path.join(OUTPUT_DIR, "M", current_image_name + "_%02d.png" % (idm)), masks["M"] * 255)
        cv2.imwrite(os.path.join(OUTPUT_DIR, "M'", current_image_name + "_%02d.png" % (idm)), masks["M'"] * 255)
        cv2.imwrite(os.path.join(OUTPUT_DIR, "P", current_image_name + "_%02d.png" % (idm)), masks["P"] * 255)
        cv2.imwrite(os.path.join(OUTPUT_DIR, "H'", current_image_name + "_%02d.png" % (idm)), masks["H'"] * 255)
        cv2.imwrite(os.path.join(OUTPUT_DIR, "flow_color", current_image_name + "_%02d.png" % (idm)), flow_color)
        plt.imsave(os.path.join(OUTPUT_DIR, "depth_color", current_image_name + "_%02d.png" % (idm)),
                   1. / depth[0].detach().numpy(), cmap="magma")
        if args.segment:
            plt.imsave(os.path.join(OUTPUT_DIR, "instances_color", "95022_%02d.png" % (idm)), instances_mask,
                       cmap="magma")
